  ...
  <div className="flex justify-center">
   {Type === "button" ? (
    ...
    >
     ...
     <span className="relative transition-colors duration-300 delay-200 group-hover:text-white">{children}</span>
    </Button>
   ) : (
    <div className="flex justify-center">
     <Button
      aria-label={Title}
      onClick={Click}
      className={`flex items-center space-x-2 hover:text-SecondaryColor transition duration-300 p-0  ${className}`}
      variant={"link"}
     >
      <span>{children}</span>
      <ArrowLeftIcon />
