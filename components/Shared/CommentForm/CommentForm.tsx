...
export default function CommentForm({parentId = null, onSubmit}: any) {
 ...
 return (
  <form onSubmit={handleSubmit(submitForm)} className="mt-4 space-y-2">
   <input className="w-full rounded border p-2" placeholder="Your name" type="text" id="name" {...register("name")} />
   <textarea
   ...
   <Button
    ...
    Type={"button"}
    className="mt-2 bg-bg-button hover:bg-bg-button-hover text-dark-color-Font font-bold py-2 px-4 rounded"
   />
  </form>
 );
}
